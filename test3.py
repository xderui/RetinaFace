...
if __name__ == '__main__':
    ...
    emotion_net = seresnet101()
    emotion_net.load_state_dict(torch.load('weights/E6_acc_0.8422.pth', map_location=lambda storage, loc: storage),
                                strict=False)
    emotion_net.eval()
    emotion_net = emotion_net.cpu()
    ...
    while(True):
        ...
        with torch.no_grad():
            ...
            for b in dets:
                ...
                crop_img = cv2.resize(gray[int(b[1]):int(b[3]),int(b[0]):int(b[2])],(48,48),interpolation=cv2.INTER_AREA)
